            ...
            for i in range(start, end + future_frame_num):
                ...
                for box_idx, instance_idx in enumerate(instance_inds):
                    ...
                    def run(future_traj=None,
                            ...
                            past_boxes=None,
                            **kwargs):
                        box = utils.get_box_from_array(gt_bboxes_3d[box_idx]) # convert to nuscenes box angle -> rotation 中心坐标还是底面中心 相当于所有box下移h/2
                        box = utils.get_transform_and_rotate_box(box, l2e_t, l2e_r) # 转到历史帧的ego coordinate
                        box = utils.get_transform_and_rotate_box(box, e2g_t, e2g_r) # 转到global coordinate
                        box = utils.get_transform_and_rotate_box(box, cur_e2g_t, cur_e2g_r, reverse=True) # 转到当前帧的ego coordinate 也就是将历史和未来每一帧的boxes的转换到当前帧的自车坐标系下
                        point = box.center # （x, y, z + h/2）才是真实的中心坐标
                        ...
                        if i < end:
                            ...
                            past_boxes[i - start, :7] = utils.get_array_from_box(box) # rotation -> angle 又转成SECOND格式了
                        else:
...
#     dataset = NuScenesTrackDatasetRadar(data_root='/root/ViP3D/data/nuscenes/', ann_file='/root/ViP3D/data/nuscenes/nuscenes_tracking_infos_val_mini.pkl')
#     data = dataset.get_data_info(0)
#     from ...tools.visualize import Visualizer
#     vis = Visualizer()
#     imgs = vis.load_images(data['img_filename'])
#     lidar2imgs = data['lidar2img']
#     boxes = data['ann_info']['gt_bboxes_3d']
#     vis.lidar_boxes2images(imgs, lidar2imgs, boxes)
